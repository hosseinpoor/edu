...
$sql = "SELECT title, holdingDays, cost, course.courseId , discountId , verify FROM orders INNER JOIN course WHERE studentMail = '" . $_GET['mail'] . "' AND (status = 1 OR status = 3) AND active = 1 AND course.courseId = orders.courseId ORDER BY course.courseId DESC";
$result = mysqli_query($db, $sql);
$msg .= "<table border='1'>";
