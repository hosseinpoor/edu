...
<div class="content <?php echo ($_SESSION["isCollapse"]=='true')? 'ac' : '' ?>">
    ...
    echo "</a></div>";
    $sql = "SELECT title, holdingDays, cost, course.courseId , discountId , receipt, verify , orderId , file FROM orders INNER JOIN course WHERE studentMail = '" . $_GET['mail'] . "' AND (status = 1 OR status = 3) AND active = 1 AND course.courseId = orders.courseId  ORDER BY course.courseId DESC LIMIT 10 OFFSET " . getOffset();
    $result = mysqli_query($db, $sql);
    ...
    <?php
    include("pager.php");
    $sql = "SELECT title, holdingDays, cost, course.courseId , discountId FROM orders INNER JOIN course WHERE studentMail = '" . $_GET['mail'] . "' AND (status = 1 OR status = 3) AND active = 1 AND course.courseId = orders.courseId";
    createPager($sql, $db);
    ?>
